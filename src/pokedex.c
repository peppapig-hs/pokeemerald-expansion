...
static bool8 CalculateMoves(void)
{
    ...
    //HM moves
    for (j = 0; j < NUM_HIDDEN_MACHINES; j++)
    {
        if(CanSpeciesLearnTMHM(species, NUM_TECHNICAL_MACHINES + j))
        {
            sStatsMoves[movesTotal] = ItemIdToBattleMoveId(ITEM_HM01_CUT + j);
            movesTotal++;
            sStatsMovesTMHM_ID[numTMHMMoves] = (ITEM_HM01_CUT + j);
            numTMHMMoves++;
        }
    }
