...
static u8 GetMedianLevelInPlayerParty(void);
static void CreateMonLatestEvolution(struct Pokemon *mon, u16 species, u8 level, u8 fixedIV, u32 personalityValue);
...
static u8 CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum, bool8 firstTrainer)
{
    ...
    u8 fixedIV, level;
    s32 i, j, temp;
    u8 monsCount, trainerLvl[PARTY_SIZE];
    u8 trainerMedian, playerMedian = GetMedianLevelInPlayerParty();
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            {
            case 0:
                trainerLvl[i] = gTrainers[trainerNum].party.NoItemDefaultMoves[i].lvl;
                break;
            case F_TRAINER_PARTY_CUSTOM_MOVESET:
                trainerLvl[i] = gTrainers[trainerNum].party.NoItemCustomMoves[i].lvl;
                break;
            case F_TRAINER_PARTY_HELD_ITEM:
                trainerLvl[i] = gTrainers[trainerNum].party.ItemDefaultMoves[i].lvl;
                break;
            case F_TRAINER_PARTY_CUSTOM_MOVESET | F_TRAINER_PARTY_HELD_ITEM:
                trainerLvl[i] = gTrainers[trainerNum].party.ItemCustomMoves[i].lvl;
                break;
            }
        }

        for (i = 0; i < monsCount; i++)
            for (j = 0; j < monsCount - i - 1; j++)
                if (trainerLvl[j] > trainerLvl[j + 1])
                    SWAP(trainerLvl[j], trainerLvl[j + 1], temp);
        
        trainerMedian = trainerLvl[monsCount / 2];
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            case 0:
            {
                ...
                level = min(MAX_LEVEL, partyData[i].lvl + min(MAX_LEVEL_DIFF, max(0, playerMedian - trainerMedian)));
                CreateMonLatestEvolution(&party[i], partyData[i].species, level, fixedIV, personalityValue);
                break;
            }
            case F_TRAINER_PARTY_CUSTOM_MOVESET:
            {
...
{
    u8 i, j, temp;
    u8 count, level[PARTY_SIZE];

    for (count = 0; count < PARTY_SIZE; count++)
    {
        if (GetMonData(&gPlayerParty[count], MON_DATA_SPECIES) == SPECIES_NONE)
            break;
        level[count] = GetMonData(&gPlayerParty[count], MON_DATA_LEVEL);
    }

    for (i = 0; i < count; i++)
        for (j = 0; j < count - i - 1; j++)
            if (level[j] > level[j + 1])
                SWAP(level[j], level[j + 1], temp);

    return level[count / 2];
}
...
static void CreateMonLatestEvolution(struct Pokemon *mon, u16 species, u8 level, u8 fixedIV, u32 personalityValue)
{
    ...
    do
    {
        CreateMon(mon, species, level, fixedIV, TRUE, personalityValue, OT_ID_RANDOM_NO_SHINY, 0);
        evolvedSpecies = GetEvolutionTargetSpecies(mon, EVO_MODE_NORMAL, ITEM_NONE, SPECIES_NONE);
        if (evolvedSpecies != SPECIES_NONE)
            species = evolvedSpecies;
    } while(evolvedSpecies != SPECIES_NONE);
}

#undef MAX_LEVEL_DIFF
